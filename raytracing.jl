...
include("vec3.jl")
include("ray.jl")
include("hittables/hittable.jl")
include("interval.jl")
include("aabb.jl")
include("hittables/hittable_list.jl")
include("hittables/bvh.jl")
include("camera.jl")
...
# add!(world, Sphere(point3(0, -1000, 0), 1000.0, ground_material))
...
#         center_point = point3(a + 0.9 * rand(), 0.2, b + 0.9 * rand())

#         if norm(center_point - point3(4, 0.2, 0)) > 0.9
#             sphere_material = nothing

#             if choose_mat < 0.8
#                 center2 = center_point + vec3(0, 0.5 * rand(), 0);
#                 # Diffuse
#                 albedo = random_vec()
#                 sphere_material = Lambertian(albedo)
#                 add!(world, Sphere(center_point, center2, 0.2, sphere_material))
#             elseif choose_mat < 0.95
#                 # Metal
#                 albedo = random_vec()
#                 fuzz = rand_float(0.0, 0.5)
#                 sphere_material = metal(albedo, fuzz)
#                 add!(world, Sphere(center_point, 0.2, sphere_material))
#             else
#                 # Glass
...
# Camera
cam = Camera(aspect_ratio=16.0 / 9.0, image_width=400, samples_per_pixel=100, max_depth=50, vfov=20.0, lookfrom=point3(13, 2, 3), lookat=point3(0, 0, 0), vup=vec3(0, 1, 0), defocus_angle=0.6, focus_dist=10.0)
